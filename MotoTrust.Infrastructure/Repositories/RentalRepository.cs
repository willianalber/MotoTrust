...
public class RentalRepository : BaseRepository<Rental>, IRentalRepository
{
    ...
    public async Task<IEnumerable<Rental>> GetRentalsByEntregadorIdAsync(string entregadorId)
    {
        return await _dbSet
    ...
    public async Task<IEnumerable<Rental>> GetRentalsByMotoIdAsync(string motoId)
    {
        return await _dbSet
